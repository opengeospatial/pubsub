...
The scope of the {titletext} is confined to enabling basic publish/subscribe functionality for resources from existing and future OGC API Stadnards. New filtering techniques will not be addressed, but adaptation of existing filtering mechanisms may be included. Advanced concepts such as pubsub security and Complex Event Processing are out of scope. Any other requirement or functionality the SWG decides to be not contributing to core publish/subscribe functionality is also out of scope.
